    ...
    if html:
        ...
        if response.status_code == 200:
            ...
            def retriever(query):
                ...
                context = retrieval.invoke(query) 
                return '\n\n'.join([i.page_content for i in context])
            prompting = """You are a helpful AI assistant. Your purpose is to answer questions based on the given context: {context}. Respond strictly according to the instructions provided. This is the memory context: {chat_history}. Please ensure your answers are relevant and accurate.""" + system_prompt
            prompt_template = ChatPromptTemplate.from_template(prompting)
